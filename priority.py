...
def adjust_for_user_defined_breaks(ko_start_time, product_duration, user_defined_breaks,dates,funname=None):
    ...
    if success:
        ...
        # print("in success")
        # print(matching_date[0].UDF)
        # print('matched date')
        # print(matching_date[0].date)
    ...
    for each_date in dates:
        ...
        # Check if scheduling from ko_start_time would overlap with the break
            if ko_start_time > break_end and ko_start_time.date()==break_end.date() and ko_start_time.date()==each_date.date and each_date.UDF==1:
                ...
                # if new_ko==datetime.datetime(2024,3,14,0,0,0):
                # print("in UDF section")
                # print(f"each date having 1:{each_date.date}")
                ...
                # print(f'final ko start time {ko_start_time}')
                setattr(each_date,'UDF',0)
                # print(funname)

                # exit()

    # if ko_start_time.hour>12 and ko_start_time.day>=14:
    #     print("hour")
    #     print(ko_start_time)
    #     exit()
...
def schedule_manager(Class_Data,dates,master_sunday_data,optimizer,see_future,dat2=None,arg=None):
    ...
    for each_date_object in dates:
        ...
        print(f'currnet date is {elements}')
        ...
        # print(each_date_object.every_line_break_pattern)
        #lets unpack the dictionary so that we can assign the values
        ...
        for eachline in available_lines:
            ...
            if eachline=='MK' :
                if elements.day_name()=='Sunday':
                    continue
                if FK.NO_six_dict==1 or KO.NO_six_dict==1:
                    total_data2=[ele for ele in total_data2 if 'NO6充填機指定' not in getattr(ele,'特記事項')]
                    far_future1=[ele for ele in far_future1 if 'NO6充填機指定' not in getattr(ele,'特記事項')]
                ...
                if elements.day_name()=='Sunday':
                    continue
                if MK.NO_six_dict==1 or KO.NO_six_dict==1:
                    total_data2=[ele for ele in total_data2 if 'NO6充填機指定' not in getattr(ele,'特記事項')]
                    far_future1=[ele for ele in far_future1 if 'NO6充填機指定' not in getattr(ele,'特記事項')]
                ...
                if MK.NO_six_dict==1 or FK.NO_six_dict==1:
                    total_data2=[ele for ele in total_data2 if 'NO6充填機指定' not in getattr(ele,'特記事項')]
                    far_future1=[ele for ele in far_future1 if 'NO6充填機指定' not in getattr(ele,'特記事項')]
            ...
            #these are the prefixes of saisyu gentei for which renzoku of only of its own type is possible
            ...
            #There is a only one 初回限定 which can not be renzokued with any other 初回限定
            # prefix_with_only_one_syokai_gentei=['LAGﾏ-ｶﾞﾘﾝ']

            #lets find the syokai getaei of this product only if it has deadline near to manufacturing deadline
            first_in_subete_group=[ele for ele in subete_group if  ele.first==1]
            first_in_current_group=[ele for ele in current_group if  ele.first==1]
            first_in_future_groups=[ele for ele in future_groups if  ele.first==1]

            if len(first_in_subete_group)>0:
                reorder_first_by_deadline = sorted(first_in_subete_group,key=lambda x:x.納期_copy)

                if getattr(reorder_first_by_deadline[0],'品名').startswith('LAGﾏ-ｶﾞﾘﾝ'):
                    first_in_subete_group=[ele for ele in first_in_subete_group if getattr(ele,'品名').startswith('LAGﾏ-ｶﾞﾘﾝ')]
                    first_in_current_group=[ele for ele in first_in_current_group if getattr(ele,'品名').startswith('LAGﾏ-ｶﾞﾘﾝ')]
                    first_in_future_groups=[ele for ele in first_in_future_groups if getattr(ele,'品名').startswith('LAGﾏ-ｶﾞﾘﾝ')]

                    subete_group=first_in_subete_group+[ele for ele in subete_group if ele.first!=1]
                    current_group=first_in_current_group+[ele for ele in current_group if ele.first!=1]
                    future_groups=first_in_future_groups+[ele for ele in future_groups if ele.first!=1]

                else:
                    subete_group=  [ele for ele in subete_group if not getattr(ele,'品名').startswith('LAGﾏ-ｶﾞﾘﾝ')]
                    current_group= [ele for ele in current_group if not getattr(ele,'品名').startswith('LAGﾏ-ｶﾞﾘﾝ')]
                    future_groups= [ele for ele in future_groups if not getattr(ele,'品名').startswith('LAGﾏ-ｶﾞﾘﾝ')]

            elif len(first_in_current_group)>0:
                reorder_first_by_deadline=sorted(first_in_current_group,key=lambda x:x.納期_copy)
            ...
            elif len(first_in_future_groups):
                reorder_first_by_deadline=sorted(first_in_future_groups,key=lambda x:x.納期_copy)
            ...
            if eachline=='MK' and elements.day_name()!='Sunday':
                ...
                # Egroup_bool = [el for el in E_Group if getattr(el,'納期_copy')== elements and getattr(el,'get_used')==0]
                ...
                #this data is to make sure that before allocating premium ace
                #make sure that there are no data which has planning day as deadline
                current_day_data=[el for el in arerugen_check_list if getattr(el,'納期_copy')==elements and getattr(el,'get_used')==0]
                ...
                if len(e1_group)==0:
                    ...
                    if len(mk_tokusyouhin)==0:
                        mk_tokusyouhin=[el for el in current_group if getattr(el,'MK特殊品')=='○' and getattr(el,'get_used')==0 ]#and getattr(el,'納期_copy')<=elements+timedelta(days=10)
                    if len(mk_tokusyouhin):#[mk_tokusyouhin[0]]
                ...
                elif len(e1_group)>0 and len(premium_ace_in_general)>0 and len(current_day_data)==0:
                ...
                elif len(e1_group)>0 and len(premium_ace3_today)>0:
                    #sorting premium-ace in ascending order of deadline
                    premium_ace_in_general=sorted(premium_ace_in_general, key=lambda x: x.納期_copy)
                ...
                elif not premium_star and len(premium_ace3_today)>0:#len(premium_ace3_today)
                    #sorting premium-ace in ascending order of deadline
                    premium_ace_in_general=sorted(premium_ace_in_general, key=lambda x: x.納期_copy)
                    premium_ace_list,counter,MK_line_total_time,allowable_time=premium_ace_allocation(MK_line_total_time,MK,premium_ace_in_general,counter,allowable_time,elements,MK_break_duration_list,MK_break_start)
                    MK_LIST=MK_LIST+premium_ace_list
                ...
                elif not premium_star and len(premium_ace_in_general)>0 and len(current_day_data)==0:#len(premium_ace3_today)
                    #sorting premium-ace in ascending order of deadline
                    premium_ace_in_general=sorted(premium_ace_in_general, key=lambda x: x.納期_copy)
                    premium_ace_list,counter,MK_line_total_time,allowable_time=premium_ace_allocation(MK_line_total_time,MK,premium_ace_in_general,counter,allowable_time,elements,MK_break_duration_list,MK_break_start)
                    MK_LIST=MK_LIST+premium_ace_list
            ...
            #+mk_list+
                ...
                #     total_data2=[ele for ele in total_data2 if 'NO6充填機指定' not in getattr(ele,'特記事項')]
                #     far_future1=[ele for ele in far_future1 if 'NO6充填機指定' not in getattr(ele,'特記事項')]
                todays_MK=[ele for ele in MK_LIST if ele.生産日==elements.date()]
                todays_MK_withno6_flag=[ele for ele in todays_MK if 'NO6充填機指定' in getattr(ele,'特記事項')]
                if len(todays_MK_withno6_flag):
                    MK.NO_six_dict=1
                ...
                todays_FK=[ele for ele in FK_LIST if ele.生産日==elements.date()]
                todays_FK_withno6_flag=[ele for ele in todays_FK if 'NO6充填機指定' in getattr(ele,'特記事項')]
                if len(todays_FK_withno6_flag):
                    FK.NO_six_dict=1
                ...
                if KO.start_time<KO.start_time.replace(hour=23,minute=0) and  KO.start_time.date() not in dat2  :#and len(ko_list)<=max_kolist
                    ...
                    for ele in new_arerugen_check_list:
                        ...
                        for dle in MB_orimpia:
                            ...
                            if  T1 and T2 and T3 and total_minutes+int(getattr(dle,'KO流量_time'))<1380 :#and len(ko_list)<=max_kolist
                                ...
                                product_duration = timedelta(minutes=getattr(dle, 'KO流量_time') + KO.cleaning_time)
                                # Adjust KO.start_time to avoid user-defined breaks
                                KO.start_time = adjust_for_user_defined_breaks(KO.start_time, product_duration, user_defined_breaks,dates, funname='c')
                        ...
                        if  total_minutes+KO.cleaning_time+int(getattr(ele,'KO流量_time'))<1380 and getattr(ele,'納期_copy').date()>=KO.start_time.date() and KO.start_time+timedelta(minutes=KO.cleaning_time)+timedelta(minutes=getattr(ele,'KO流量_time'))<=KO.end_time and  next_DaY not in dat2 and TT:#and len(ko_list)<=max_kolist
                            ...
                            product_duration = timedelta(minutes=getattr(ele, 'KO流量_time') + KO.cleaning_time)
                            # Adjust KO.start_time to avoid user-defined breaks
                            KO.start_time = adjust_for_user_defined_breaks(KO.start_time, product_duration, user_defined_breaks,dates, funname='d')
                ...
                if len(ko_list)>0 :#and elements.month<=4 and elements.day<15
                    if datetime.datetime.strptime(ko_list[-1].end, "%Y-%m-%d %H:%M:%S").hour<=12:
                        ...
                        if len(KO_LIST):
                            ...
                            for each_ele in dates:
                                if each_ele.date== KO.start_time.date():
                                    setattr(each_ele,'UDF',1)
                            ...
                            for ele in new_arerugen_check_list:
                                ...
                                for dle in MB_orimpia:
                                    ...
                                    if  T1 and T2 and T3 and total_minutes+int(getattr(dle,'KO流量_time'))<1380  :#and len(ko_list)<=max_kolist
                                        ...
                                        product_duration = timedelta(minutes=getattr(dle, 'KO流量_time') + KO.cleaning_time)
                                        # Adjust KO.start_time to avoid user-defined breaks
                                        KO.start_time = adjust_for_user_defined_breaks(KO.start_time, product_duration, user_defined_breaks,dates)
                                ...
                                if  total_minutes+KO.cleaning_time+int(getattr(ele,'KO流量_time'))<1490 and getattr(ele,'納期_copy').date()>=KO.start_time.date() and KO.start_time+timedelta(minutes=KO.cleaning_time)+timedelta(minutes=getattr(ele,'KO流量_time'))<=KO.end_time and  next_DaY not in dat2 and TT:#and len(ko_list)<=max_kolist
                ...
                todays_KO=[ele for ele in KO_LIST if ele.生産日==elements.date()]
                todays_KO_withno6_flag=[ele for ele in todays_KO if 'NO6充填機指定' in getattr(ele,'特記事項')]
                if len(todays_KO_withno6_flag):
                    KO.NO_six_dict=1
