...
for ind,row in df_east_filtered.iterrows():
    ...
    #
    #magarin data preprocess
# for i in range(0,int(row['増ﾊﾞｯﾁ数'])):

    weight=row['増t']*1000*int(row['増ﾊﾞｯﾁ数'])#row['増t']*row['増ﾊﾞｯﾁ数']*1000
...
order_data.to_csv('shortening_3-5-order_data_2024_3_月.csv',index=False,encoding='cp932')